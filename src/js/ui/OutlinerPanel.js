...
import { SHAPE_ICONS } from "../utils/Constants.js";
...
class OutlinerPanel extends EventEmitter {
  ...
  refresh() {
    ...
    const ungroupedObjects = allObjects.filter(
      (obj) => !groupedObjectIds.has(obj.uuid) && obj.visible
    );
  ...
  createTreeItem(object, level = 0) {
    ...
    // Content
    let icon = '<ss-icon icon="cube" thickness="2.2"></ss-icon>';
    if (isGroup) {
      icon = isExpanded
        ? '<ss-icon icon="chevron-down" thickness="2.2"></ss-icon>'
        : '<ss-icon icon="chevron-right" thickness="2.2"></ss-icon>';
    ...
    item.innerHTML = `
            <div class="swk-outliner-item-content" style="padding-left: ${indent}px;">
                ${
                  isGroup
                    ? `<span class="swk-outliner-toggle" data-object-id="${object.userData.id}">${icon}</span>`
                    : `<span class="swk-outliner-icon">${icon}</span>`
                }
                <span class="swk-outliner-name">${object.name}</span>
                <span class="swk-outliner-type">${
                  isGroup ? "Group" : object.userData.shapeType || ""
                }</span>
            </div>
        `;
  ...
  getShapeIcon(type) {
    ...
    if (SHAPE_ICONS[type]) {
      return `<ss-icon icon="${SHAPE_ICONS[type]}" thickness="2.2"></ss-icon>`;
    }

    // Fallback for groups or unknown types
    return `<ss-icon icon="cube" thickness="2.2"></ss-icon>`;
  }
  ...
  updateSelection(selectedObjects) {
    ...
    const items = this.container.querySelectorAll(
      ".swk-outliner-item-selected"
    );
    items.forEach((item) =>
      item.classList.remove("swk-outliner-item-selected")
    );
