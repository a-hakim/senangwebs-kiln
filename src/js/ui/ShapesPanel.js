...
class ShapesPanel extends EventEmitter {
  ...
  constructor(container, swkInstance, config) {
    ...
    this.shapes = [
      ...
      {
        type: SHAPE_TYPES.HALF_CYLINDER,
        label: "Half Cylinder",
        icon: "cylinder-half",
      },
      {
        type: SHAPE_TYPES.HALF_SPHERE,
        label: "Half Sphere",
        icon: "sphere-half",
      },
      { type: SHAPE_TYPES.HEART, label: "Heart", icon: "heart" },
      { type: SHAPE_TYPES.TORUS_KNOT, label: "Torus Knot", icon: "torus-knot" },
      {
        type: SHAPE_TYPES.TETRAHEDRON,
        label: "Tetrahedron",
        icon: "tetrahedron",
      },
      { type: SHAPE_TYPES.OCTAHEDRON, label: "Octahedron", icon: "octahedron" },
      {
        type: SHAPE_TYPES.ICOSAHEDRON,
        label: "Icosahedron",
        icon: "icosahedron",
      },
      {
        type: SHAPE_TYPES.DODECAHEDRON,
        label: "Dodecahedron",
        icon: "dodecahedron",
      },
    ];
  ...
  render() {
    ...
    this.shapes.forEach((shape) => {
      ...
      button.innerHTML = `
                <span class="swk-shape-icon"><ss-icon icon="${
                  SHAPE_ICONS[shape.type]
                }" thickness="1.8"></ss-icon></span>
