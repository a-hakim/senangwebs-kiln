...
class SWK extends EventEmitter {
  ...
  onCanvasClick(event) {
    ...
    this.objects.forEach((obj) => {
      if (
        obj.visible &&
        !obj.userData.isGroupResult &&
        !this.groupManager.isGrouped(obj)
      ) {
    ...
    const canvas = this.renderer.getCanvas();
    const intersection = this.picker.pickFromEvent(
      event,
      canvas,
      selectableObjects,
      true
    ...
    if (intersection) {
      ...
      if (selectedObjects.length > 0) {
        ...
        const primarySelection =
          selectedObjects.find((obj) =>
            this.groupManager.isGroupContainer(obj)
          ) || selectedObjects[0];
        if (
          selectedObjects.length === 1 ||
          this.groupManager.isGroupContainer(primarySelection)
        ) {
  ...
  _duplicateGroupInternal(groupContainer, captureState = false) {
    ...
    // Create new group
    const newGroupContainer = this.groupManager.createGroup(
      newGroupObjects,
      `${group.name} (Copy)`
    );
  ...
  isGroupContainer(object) {
    return this.groupManager
      ? this.groupManager.isGroupContainer(object)
      : false;
  }
  ...
  getGroupChildren(groupContainer) {
    return this.groupManager
      ? this.groupManager.getGroupChildren(groupContainer)
      : [];
  }
